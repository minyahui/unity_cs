...
function LateUpdate(){
	...
	if (health <= 0){  //Die blend.
		dieBlend = 1.0;
		if (hitDirection.z > 0){
			GetComponent.<Animation>()["soldierDieFront"].time = Time.time - deathTime;
			...
			GetComponent.<Animation>()["soldierDieBack"].time = Time.time - deathTime;
