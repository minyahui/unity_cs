...
public class TwoLongTap : MonoBehaviour {
	...
	void On_LongTapStart2Fingers( Gesture gesture){
		...
		if (gesture.pickObject == gameObject){
			gameObject.GetComponent<Renderer>().GetComponent<Material>().color = new Color( Random.Range(0.0f,1.0f),  Random.Range(0.0f,1.0f), Random.Range(0.0f,1.0f));
		}
	}
	...
	void On_LongTapEnd2Fingers( Gesture gesture){
		// Verification that the action on the object
		if (gesture.pickObject == gameObject){
			gameObject.GetComponent<Renderer>().GetComponent<Material>().color = new Color(1f,1f,1f);
